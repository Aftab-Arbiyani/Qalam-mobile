/// Shared cursor-pagination state + engine (docs/40 §8.3, §36). EVERY feed and
/// bookmark timeline reuses this — there is no per-feed pagination code. A feed
/// controller holds an `AsyncValue<PagedListState<T>>`: page-1 loading/error is the
/// `AsyncValue` (→ skeleton / error view), while subsequent pages live inside the
/// state ([isLoadingMore], [loadMoreFailure]) so the accumulated list stays painted
/// while the next page loads (docs/41 §17). Cursors stay opaque and out of the UI.
library;
...
/// The pagination engine shared by every feed controller. It takes one callback —
...
class FeedPaginator<T> {
  const FeedPaginator(this._fetch);
