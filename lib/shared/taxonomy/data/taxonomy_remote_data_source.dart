/// Shared taxonomy remote source (docs/40 §17.1). The only place taxonomy touches
/// the wire. Fetches the language + genre option lists from the frozen discovery
/// endpoints (`GET /discover/languages|genres`) — the contract-bound source since
/// `v1` has no dedicated "list taxonomy" endpoint. `v1` taxonomy is a small fixed
/// set, so one max-size page is fetched (documented limitation, docs/40 §45).
library;
...
class TaxonomyRemoteDataSource {
  TaxonomyRemoteDataSource(this._api);
