...
void main() {
  ...
  test('loads conversations newest-first', () async {
    final ProviderContainer c = await load(
      FakeAiRepository(conversations: <AiConversationSummary>[_summary('c1'), _summary('c2')]),
    );
    expect(c.read(conversationsControllerProvider).asData!.value.items.length, 2);
  });
  ...
  test('pinning persists and sorts pinned rows first', () async {
    ...
    final ConversationsState state = c.read(conversationsControllerProvider).asData!.value;
    expect(state.isPinned('c2'), isTrue);
    expect(state.ordered.first.id, 'c2');
    // Persisted on-device.
    expect(c.read(promptLibraryStoreProvider).pinnedConversationIds(), contains('c2'));
  });

  test('delete removes the row and calls the platform', () async {
    final FakeAiRepository fake =
        FakeAiRepository(conversations: <AiConversationSummary>[_summary('c1'), _summary('c2')]);
    final ProviderContainer c = await load(fake);
    final bool ok = await c.read(conversationsControllerProvider.notifier).delete('c1');
    expect(ok, isTrue);
    expect(fake.deletedConversationIds, contains('c1'));
    expect(
      c.read(conversationsControllerProvider).asData!.value.items.map((AiConversationSummary e) => e.id),
      <String>['c2'],
    );
  });
}
